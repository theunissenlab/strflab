function g=cnnGradTimeDomain(strf,datIdx)
...
[strf,resp_strf,fm,hl,a] = cnnFwd(strf, datIdx);
...
if isempty(hl)
    ...
    goutw = fm'*delout;
    goutb = sum(delout, 1);
    delfm = delout*strf.outw';
    delfm = delfm.*(1.0 - fm.*fm);
else
    hl(find(isnan(hl))) = 0;
    goutw = hl'*delout;
    goutb = sum(delout, 1);
    delhid = delout*strf.outw';
    delhid = delhid.*(1.0 - hl.*hl);
    ghlw = fm'*delhid;
    ghlb = sum(delhid, 1);
    delfm = delhid*strf.hlw';
    delfm = delfm.*(1.0 - fm.*fm);
end
