function varargout = df_do_cached_calc(function_name,varargin)
...
if ~exist('df_dir_of_caches.m','file')
    disp('Function "df_do_cached_calc" cannot find the file "df_dir_of_caches.m", which tells it where to put its saved intermediate results.  Evaluating anyways...');
    ...
    if strcmp(cached_dir,'null')
        ...
        if ~exist(cached_dir,'dir')
            disp(['Warning from function "do_cached_calc": I was told by function "dir_of_caches" that directory "' char(10) cached_dir '" is where I should save my data' ...
        ...
        function_text = load_function_text(function_name);

        %b = which('get_strfpak_version');
        if 0%length(b) > 0
            strfpak_version = get_strfpak_version;
        else
        ...
        %  strfpak version.
        global the_checksum
        the_checksum = checksum(function_text,strfpak_version,varargin);
        %Why does MATLAB choke on "varargout = feval(function_name,varargin)"?
        %With this limitation, I can't even make this hack modular.
        %This next bit is a hack which gets around this limitation.
        if nargout == 0
        ...
        to_eval = [to_eval 'do_cached_calc_checksum_known(function_name,the_checksum'];
        for jj = 1:(nargin-1)
            to_eval = [to_eval ',varargin{' num2str(jj) '}'];
        end
        to_eval = [to_eval ');'];
