...
%  3/16/2011 FET. Added the automatic cuttof for info calculation when lower
%  bound is negative.
%  3/16/2011 FET. Added a debug plot option
...
function cStruct = compute_coherence_mean(modelResponse, psth, sampleRate, freqCutoff, windowSize)
    ...
    debug_plot = 0;  % Set to one to enable plotting
    ...
    if (debug_plot)
        ...
        plot([cStruct.f(cutoffIndex) cStruct.f(cutoffIndex)], [0 max(cStruct.cUpper)], 'k--');
        hold off;
    end
