function [strf,options]=trnThreshGradDesc(strf,datIdx,options,varargin)
...
% --------------------
global globDat
optDeflt.funcName='trnThreshGradDesc';
optRange.funcName={'trnThreshGradDesc'};
...
for iter=1:iterstouse
  ...
  if iter~=1
    ...
    % else
      grad = grad/norm(grad);
      [d,idx] = max(abs(grad));
      tidx = find(abs(grad)>=(options.threshold.*d));
      x(tidx) = x(tidx) - options.stepSize * grad(tidx);
    % end
