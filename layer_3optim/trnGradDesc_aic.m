...
%                         1 means early stop
%                         2 mean using the AICc to stop and obtain a sparse solution
%                           (use only with coorDesc = 1)
...
% --------------------
global globDat
optDeflt.funcName='trnGradDesc_aic';
optRange.funcName={'trnGradDesc_aic'};
...
if options.earlyStop==1 & isempty(varargin)
    warning(sprintf('You must specify an Early Stopping Index if you want to use Early Stopping'));
    return
elseif options.earlyStop==2 & ~isempty(varargin)
    warning(sprintf('You do not need an Early Stopping Set when using the AICc Stopping Criteria'));
    return
end

if options.coorDesc==0 & options.earlyStop==2
    warning(sprintf('Only use the AICc Stopping Criteria with Coordinate Descent'));
    return
end

if options.earlyStop==2 & options.errLastN > 2;
    warning(sprintf('When using AICc for stopping it is recommended that errLastN be around 2'));
    pause(2)
end
...
for iter=1:iterstouse
  ...
  elseif options.earlyStop==2
    k = sum(x~=0);
    n = size(datIdx,2);
    stoperr = 2*k + n*log(2*esterr/n); %% AIC assumes linear model with RSS error function
    stoperr = stoperr + (2*k*(k+1))/(n-k-1); %% AICc
    options.diagnostics.errs(2,iter) = stoperr;
  end
    ...
    if iter==1 || mod(iter,options.display)==0
      if options.earlyStop==0
        ...
        if options.is1D
            ...
            h1 = bar(x); title('Parameter magnitudes');
        end
        ...
        h3 = plot((1+options.nDispTruncate):length(options.diagnostics.errs),options.diagnostics.errs(2,(1+options.nDispTruncate):end),'-'); 
        if options.earlyStop==1
            title('Early Stopping Set Error');
        elseif options.earlyStop==2
            title('AICc Value');
        end
      end
  ...
  if (options.nIter >= 0) || (options.earlyStop==0 && esterr < esterr_min) || (options.earlyStop > 0  && stoperr < stoperr_min)
