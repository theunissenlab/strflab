function [strf,options]=trnGradDescLineSearch(strf,datIdx,options,varargin)
...
% --------------------
global globDat
optDeflt.funcName='trnGradDescLineSearch';
optRange.funcName={'trnGradDescLineSearch'};
...
optDeflt.is1D = 0;
optRange.is1D = [0 1];
optDeflt.lineSearch = 0;
optRangle.lineSearch = [0 1];
if nargin<3
  options=optDeflt;
else
  ...
  if iter~=1
    ...
    if options.coorDesc==0
      ...
      if options.lineSearch
        fpath = which('fminbnd');
        if isempty(fpath)
            error('Line search requires optimization toolbox (fminbnd not found).');
        end
        % choose an optimal step size
        minErrFunc = @(alpha) strfErrorForLineSearch(strf, datIdx, x - (alpha*grad));
        %bestStep = fminbnd(minErrFunc, 1e-8, 1e3);
        %bestStep = fminsearch(minErrFunc, options.stepSize);
        [bestStep, val, eflag] = fminsearch(minErrFunc, 1);
        %fprintf('fminsearch returned, eflag=%d, val=%f\n', eflag, val);
...
%% function used for line searching
function err = strfErrorForLineSearch(strf, datIdx, x)
    strf = strfUnpak(strf, x);
    [strf, err] = strfErr(strf, datIdx);
