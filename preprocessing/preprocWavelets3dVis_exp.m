...
%
% im_guass bugfix June 2009 by Michael Oliver
...
for ex_or_sp = 1:2
	...
	for ii=1:length(exsp.ind)
		...
		switch thisgparam(8)
		    case {0}  % spectra channels
				im_spec(:,:,thisdelays) = im_spec(:,:,thisdelays) + thisgabor*exsp.wvalue(ii);
			case {3,4,5,6}  % half-rectified channels
...
im_spec_ex = flipdim(im_spec_ex,3);
im_spec_sp = flipdim(im_spec_sp,3);
...
out.im_spec_ex = im_spec_ex;
out.im_spec_sp = im_spec_sp;
...
maxim_spec_ex = max([10e-10; abs(im_spec_ex(:))]);
maxim_spec_sp = max([10e-10; abs(im_spec_sp(:))]);
...
totalmax = max([maxim_ex maxim_sp maxim_spec_ex maxim_spec_sp maxim_linear]);
...
for ii=1:vis_t_size*moviereps
	...
	figure(7);
    subplot(3,2,1), imagesc(im_ex(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	title(sprintf('ex simple: %.3f', maxim_ex/totalmax));

	subplot(3,2,2), imagesc(im_sp(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	title(sprintf('sp simple: %.3f', maxim_sp/totalmax));
	
	subplot(3,2,3), imagesc(im_spec_ex(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	title(sprintf('ex complex: %.3f', maxim_spec_ex/totalmax));

	subplot(3,2,4), imagesc(im_spec_sp(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
	title(sprintf('sp complex: %.3f', maxim_spec_sp/totalmax));

	subplot(3,2,5), imagesc(im_linear(:,:,t_kernel), [-totalmax totalmax]);
	axis off; axis square;
