...
if params.RTAC(1) & params.RTAC(2)
    [A, idxr, idxc] = getSubMatrix([size(rawStim,1) size(rawStim,2) (params.delays + 1)], params.locality*2 +1, [1 1 params.covtime]);
    params.idxr = idxr; params.idxc = idxc;
    rawStim = reshape(rawStim, size(rawStim,1)*size(rawStim,2), size(rawStim,3))';
    
    if ~params.covtime
        stim = zeros([size(rawStim,1) size(idxr,1)+params.covsz], 'single');
        stim(:,1:params.covsz) = rawStim;
        %% Calculate the mean to subtract off
        ErawStim = mean(rawStim,1);
        rawStim = bsxfun(@minus,rawStim,ErawStim);
        stim(:,params.covsz+1:end) = rawStim(:,idxr).*rawStim(:,idxc);
    
    else
        stim = zeros([size(rawStim,1) size(idxr,1)+((params.delays + 1)*params.covsz)], 'single');
        for ii = 1:(params.delays + 1)
            stim(:,(ii-1)*params.covsz+1:ii*params.covsz) = [zeros([ii-1 params.covsz], 'single'); rawStim(1:end-ii+1,:)];
        end
        
        %% Calculate the mean to subtract off
        ErawStim = mean(rawStim,1);
        rawStim = [zeros((params.delays + 1)-1, size(rawStim,2)); rawStim];
        rawStim = bsxfun(@minus,rawStim,ErawStim);
        
        for ii = 1:1000:size(idxr,1)
            iiend = min([size(idxr,1) ii+999]);
            %% Separate row and column indexes into pixel and time indicies
            [rr,tr] = ind2sub([size(rawStim,2) (params.delays + 1)], idxr(ii:iiend));
            [cc,tc] = ind2sub([size(rawStim,2) (params.delays + 1)], idxc(ii:iiend));
            
            %% Create pixel and time indices across all samples
            rridx = repmat(rr, [1 size(rawStim,1)-(params.delays + 1)+1]);
            ccidx = repmat(cc, [1 size(rawStim,1)-(params.delays + 1)+1]);
            
            tridx = bsxfun(@minus, repmat(uint32([(params.delays + 1):size(rawStim,1)]), [size(rr,1) 1])',tr')+1;
            tcidx = bsxfun(@minus, repmat(uint32([(params.delays + 1):size(rawStim,1)]), [size(cc,1) 1])',tc')+1;
            
            %% Get linear index into stimuli from pixel and time indicies
            indr = sub2ind(size(rawStim), tridx(:), rridx(:));
            indc = sub2ind(size(rawStim), tcidx(:), ccidx(:));
            
            stim(:,(params.delays + 1)*params.covsz+ii:(params.delays + 1)*params.covsz+iiend) = reshape(rawStim(indr).*rawStim(indc), [size(rawStim,1)-(params.delays + 1)+1 iiend-ii+1]);
...
elseif ~params.RTAC(1) & params.RTAC(2)
    ...
    if ~params.covtime        
        ...
        %% Calculate the mean to subtract off
        ErawStim = mean(rawStim,2);
        rawStim = [zeros(size(rawStim,1), (params.delays + 1)-1) rawStim];
        rawStim = bsxfun(@minus,rawStim,ErawStim);  
        stim = zeros([size(idxr,1) size(rawStim,2)], 'single');
        for ii = 1:1000:size(idxr,1)
            iiend = min([size(idxr,1) ii+999]);
            %% Separate row and column indexes into pixel and time indicies
            [rr,tr] = ind2sub([size(rawStim,1) (params.delays + 1)], idxr(ii:iiend));
            [cc,tc] = ind2sub([size(rawStim,1) (params.delays + 1)], idxc(ii:iiend));
            
            %% Create pixel and time indices across all samples
            rridx = repmat(rr, [1 size(rawStim,2)-(params.delays + 1)+1]);
            ccidx = repmat(cc, [1 size(rawStim,2)-(params.delays + 1)+1]);
            
            tridx = bsxfun(@minus, repmat(uint32([(params.delays + 1):size(rawStim,2)]), [size(rr,1) 1]),tr)+1;
            tcidx = bsxfun(@minus, repmat(uint32([(params.delays + 1):size(rawStim,2)]), [size(cc,1) 1]),tc)+1;
            ...
            stim(ii:iiend,:) = reshape(rawStim(indr).*rawStim(indc), [iiend-ii+1 size(rawStim,2)-(params.delays + 1)+1]);
            fprintf('preprocRTAC percent complete: %.1f\n', 100*iiend/size(idxr,1));
        end
    end
end
