...
optDeflt.class = 'preprocRTAC_old';
optDeflt.covtime = 0;
...
if params.RTAC(1) & params.RTAC(2)
    ...
    if ~params.covtime
        ...
        stim = zeros([size(idxr,1)+((params.delays + 1)*params.covsz) size(rawStim,2)], 'single');
        for ii = 1:(params.delays + 1)
            stim((ii-1)*params.covsz+1:ii*params.covsz,:) = [zeros([params.covsz ii-1], 'single') rawStim(:,1:end-ii+1)];
        end
        ...
        for ii = 1:1000:size(idxr,1)
            ...
            stim((params.delays + 1)*params.covsz+ii:(params.delays + 1)*params.covsz+iiend,:) = reshape(rawStim(indr).*rawStim(indc), [iiend-ii+1 size(rawStim,2)-(params.delays + 1)+1]);
